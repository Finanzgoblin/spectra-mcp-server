...
- **Monitor** pool trading volume and individual transaction activity
- **Quote** PT trades with estimated output, price impact, and slippage-adjusted minOut
...
Modular TypeScript server. Read-only — queries Spectra's existing API, Morpho's GraphQL API, and Base RPC for veSPECTRA data. No wallet, no transactions, no keys, no web3 libraries needed.
...
src/
  ...
  tools/            Layer 2: each tool description teaches domain-specific mechanics
    ...
    simulate.ts     simulate_portfolio_after_trade
